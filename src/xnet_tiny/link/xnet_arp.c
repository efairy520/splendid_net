...
typedef struct _xarp_entry_t {
    xip_addr_t ip_addr;                                     // ip地址
    uint8_t mac_addr[XNET_MAC_ADDR_SIZE];                   // mac地址
    uint8_t state;                                          // 状态位
    uint16_t ttl;                                           // 剩余时间
...
static xarp_entry_t* xarp_find_by_ip(const uint8_t *ipaddr) {
    for (int i = 0; i < XARP_CFG_TABLE_SIZE; i++) {
        if (arp_table[i].state != XARP_ENTRY_FREE && xip_addr_eq(ipaddr, arp_table[i].ip_addr.addr)) {
            return &arp_table[i];
        }
    }
    return NULL;
...
static void update_arp_entry(uint8_t *src_ip, uint8_t *mac_addr) {
    ...
    memcpy(entry->ip_addr.addr, src_ip, XNET_IPV4_ADDR_SIZE);
...
void xarp_init(void) {
    ...
    // 初始化ARP上一次扫描时间，为当前时间
    xnet_time_record(&arp_last_time);
}

// 轮询ARP表项是否超时，超时则重新请求
void xarp_poll(void) {
    // 每隔 PERIOD（1秒） 执行一次
    if (xnet_time_check_tmo(&arp_last_time, XARP_CFG_TIMER_PERIOD)) {
        for (int i = 0; i < XARP_CFG_TABLE_SIZE; i++) {
            switch (arp_table[i].state) {
                ...
                case XARP_ENTRY_RESOLVING:
                    ...
                    if (arp_table[i].ttl <= XARP_CFG_TIMER_PERIOD) {     // PENDING超时，准备重试
                        if (arp_table[i].retry_cnt-- == 0) { // 重试次数用完，回收
                            ...
                            xarp_make_request(&arp_table[i].ip_addr);
                ...
                case XARP_ENTRY_OK:
                    ...
                    if (arp_table[i].ttl <= XARP_CFG_TIMER_PERIOD) {     // OK超时，重新请求
                        xarp_make_request(&arp_table[i].ip_addr); // 想要测试，需要把虚拟机网络关闭，否则一直ok
                        arp_table[i].state = XARP_ENTRY_RESOLVING;
                        arp_table[i].ttl = XARP_CFG_ENTRY_RESOLVING_TMO;
...
xnet_status_t xarp_resolve(const xip_addr_t *ipaddr, uint8_t **mac_addr) {
    ...
    // 没有匹配到arp表项，发送arp请求
    entry = xarp_get_free_entry();
    memcpy(entry->ip_addr.addr, ipaddr->addr, XNET_IPV4_ADDR_SIZE);
